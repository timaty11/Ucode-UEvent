...
const StapperProgress = ({ completeStep, stepper, setStepper, setCurrentStep, currentStep }) => {
  ...
  return (
    <div className="flex justify-between items-center mt-20 ">
      <div className="w-[90%]">
        <ol className="flex items-center w-full">
          { svgStep.map(({ start, end, isEnd }, iter) => {
            return (
              !stepper['step' + (iter + 1)] ? (
                <StapperLine setCurrentStep={setCurrentStep} currentStep={currentStep} iter={iter + 1} step={stepper.iter} isEnd={isEnd} >
                  {' '}
                  {start}{' '}
                </StapperLine>
              ) : (
                <CompliteLine setCurrentStep={setCurrentStep} step={iter + 1} isEnd={isEnd} >
                  {' '}
                  {end}{' '}
                </CompliteLine>
              )
      ...
      <button className={`${isCompleteStep ? '' : 'opacity-80'} bg-primary-600 hover:bg-primary-700 dark:bg-primary-600 dark:hover:bg-primary-700 transition p-10 text-white font-medium rounded-lg text-sm px-5 py-2.5 text-center  `}
        onClick={(e) => {
          if (!isCompleteStep) {
            toast.dismiss();
