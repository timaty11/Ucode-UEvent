  ...
  return (
    <div className="w-1/5 relative">
      ...
      <div
        className={`${
          active ? '' : 'hidden'
        } z-20 bg-white rounded-lg absolute w-full border shadow z-10`}
      >
        {!searchEvent ? (
          <div className="p-5"></div>
        ) : searchEvent.length === 0 ? (
          <div className="text-gray-700 block px-4 py-2 text-sm hover:bg-gray-100 text-gray-900">
            No search result
          </div>
        ) : (
          searchEvent.map((item, i) => (
            <div clas>
              <a
                href={`/event/${item.id}`}
                key={i}
